...
void update_buttons() {
  for(int i = 0; i < 12; i++) {
    ...
    if (buttons[i].pressed && !buttons[i].last_pressed) buttons[i].last_pressed_time = pros::millis();
  }
}

bool check_rising(pros::controller_digital_e_t button) {
  return (buttons[button].pressed && (pros::millis() - buttons[button].last_pressed) > buttons[button].last_pressed_time);
}
...
bool check_double_press(pros::controller_digital_e_t button1, pros::controller_digital_e_t button2) {
  if (((pros::millis() - buttons[button1].last_pressed) < buttons[button1].last_pressed_time) && check_rising(button2)) {
         return true;
     }
  if (((pros::millis() - buttons[button2].last_pressed) < buttons[button2].last_pressed_time) && check_rising(button1)) {
         return true;
     }
