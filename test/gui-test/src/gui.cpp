...
void gui_init() {
  ...
  // sprintf(battery_bar_string, "%.1f%", pros::battery::get_capacity());
  ...
  //   lv_obj_set_size(shot_slider[i], 400, 30);
  //   lv_slider_set_range(shot_slider[i], 50, 100);
  //   lv_slider_set_action(shot_slider[i], shot_tuning_slider_action);
  //   if (i == 0) lv_obj_align(shot_slider[i], shot_tuning_title, LV_ALIGN_OUT_BOTTOM_LEFT, -120, 10);
  //   else lv_obj_align(shot_slider[i], shot_slider[i - 1], LV_ALIGN_OUT_BOTTOM_LEFT, -120, 10);
  //
