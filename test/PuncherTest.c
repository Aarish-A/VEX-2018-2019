...
task main()
{
	...
	while (true)
	{
		...
		if (button && !lstButton)
		{
			...
			writeDebugStreamLine("%d Start shot%d %f", npgmtime, shotCount, target);
			while( (gSensor[enc].value - startVal) < (target-offset) ) //Spin 1 rotation
			{
				//writeDebugStreamLine("%d sC:%d, targ:%f, Sensor:%d", nPgmTime, shotCount, target, gSensor[enc].value);
				sleep(10);
			}
