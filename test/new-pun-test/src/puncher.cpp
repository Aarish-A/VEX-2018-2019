...
void pun_cal() {
  ...
  while (fabs(puncherLeft.get_actual_velocity()) < 12 && (success = (millis() < timeout_time)))
	{
		printf("%d A: %f T:%f \n", pros::millis(), puncherLeft.get_actual_velocity(), puncherLeft.get_temperature());
		delay(10);
	}
  if (!success) {
		printf("disableA \n");
  ...
  delay(100);
	...
	printf("%d Pun Cal Before Tare. LeftPos:%f, RightPos:%f \n", pros::millis(), puncherLeft.get_position(), puncherRight.get_position());
  puncherLeft.tare_position();
  puncherRight.tare_position();
	printf("%d Pun Cal Done Tare. LeftPos:%f, RightPos:%f \n", pros::millis(), puncherLeft.get_position(), puncherRight.get_position());
