...
#define ANGLER_FRONT_TOP_FLAG 1700
#define ANGLER_FRONT_MID_FLAG 1280
...
task anglerStateSet()
{
	...
	//Constants for anglerHold state
	float kP = 0.095;//0.09;
...
void doubleShot(int posA, int posB, int acceptableRange, bool waitForFirstShot = true, bool waitForSecShot = true)
{
  ...
  //First shot
  anglerMoveToPos(posA, acceptableRange);
  if (waitForFirstShot)
	{
  ...
  while (gShooterState != shooterReload) sleep(10); //Start moving angler immediately after first shot applies breaking
	...
	//Second shot
  anglerMoveToPos(posB, acceptableRange);
  if (waitForSecShot)
	{
    while (gAnglerGoodCount < 5) sleep(10);
    writeDebugStreamLine("%d Done waiting for point. Angler:%d", nPgmTime, SensorValue[anglerPoti]);
...
task usercontrol()
{
	...
	while (true)
	{
		...
		{
			writeDebugStreamLine("%d Angler to pickup position", nPgmTime);
