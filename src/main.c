...
#define DECAPPER_BOTTOM 125
#define DECAPPER_TOP 3650
#define DECAPPER_MID 2250
int gBatteryLevel;
...
task decapperControl()
{
	while(true)
	{
		word power = vexRT[Ch1];
		if(vexRT[Btn8L])
		{
			if(sgn(DECAPPER_MID-SensorValue[decapperPoti])==1)
			{
				motor[decapper] = 127;
				while(SensorValue[decapperPoti]<DECAPPER_MID-150) sleep(10);
				motor[decapper]	=	0;
				sleep(100);
			}
		}
		if(abs(power)>40)
		{
			if (power > 0 && SensorValue[decapperPoti] > DECAPPER_TOP) power = 0;
			else if (power < 0 && SensorValue[decapperPoti] < DECAPPER_BOTTOM) power = 0;
		}
		else if(SensorValue[decapperPoti]<270)power=0;
		else if(SensorValue[decapperPoti]<DECAPPER_MID)power=0;

		else power = 0;
		motor[decapper] = power;
		sleep(10);
	}
}
/* LCD Controls */
