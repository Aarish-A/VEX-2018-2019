...
#pragma config(Motor,  port8,           angler,        tmotorVex393TurboSpeed_MC29, openLoop)
	...
	while (true)
	{
		...
		if (vexRt[BTN_SHOOT])//RISING(BTN_SHOOT) )
		{
			if (shooterShotCount == 0)
			{
				...
				} while(abs(gSensor[shooterEnc].velocity) >= 0.001 || abs(gSensor[shooterEnc].lstVelocity) >= 0.001)
...
task intakeAnglerTask()
{
	while (true)
	{
		if (gSensor[anglerPoti].value > 510 && gSensor[anglerPoti].value < 3037 && abs(gJoy[JOY_ANGLER].cur) > 10)
		{
