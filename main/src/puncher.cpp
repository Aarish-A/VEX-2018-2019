  ...
  if (ball_sensor.get_value() < PUN_BALL_THRESH) {
		...
		switch (pun_state) {
			...
			case PunState::Hold:
				...
				if (( shot_req_num > 0 && shot_req[shot_req_handled_num].drive_turn_handled && (fabs(angler.get_position()-shot_req[shot_req_handled_num].angle_targ) < 5 || pros::millis() > shot_req[shot_req_handled_num].angler_to) ) || auto_set_shot) {
