...
void pun_cal() {
  ...
  bool success = true;
  pun_set(-20);
  while (fabs(puncher.get_actual_velocity()) < 15 && (success = (millis() < timeout_time))) delay(10);
  if (!success) {
    pun_fatal_disable();
    return;
  }
  timeout_time = millis() + 1300;
  while (fabs(puncher.get_actual_velocity()) > 10 && (success = (millis() < timeout_time))) delay(10);
  if (!success) {
    pun_fatal_disable();
    return;
  }
