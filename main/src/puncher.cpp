  ...
  if (ball_sensor.get_value() < PUN_BALL_THRESH) {
		...
		switch (pun_state) {
			case PunState::Loading:
			{
				...
				else if(cur_err >= e_targ && cancelled)
				{
					pun_set(PUN_HOLD_PWR);
			...
			{
				if (( shot_req_num && shot_req[shot_req_handled_num].drive_turn_handled && //For Driver control - shot request must have been made, drive must have been handled & angler must have reached or timed out
