...
if (current_auto_routine == 0 && game_side == 'R')
{
  ...
  auto_set_angler_target(front_SP.top+28);
  ...
  auto_set_angler_target(front_SP.mid+7);
  pros::delay(250);
  ...
  auto_set_shot = true;
  while (auto_set_shot) pros::delay(10);
  auto_set_angler_target(front_SP.mid+4);
  pros::delay(250);
...
else if (current_auto_routine == 1 && game_side == 'R')
{
  ...
  auto_set_angler_target(front_SP.top+52);
  turn_vel( FixedAngleTarget(-57.3_deg), (200/90_deg));
  move_drive_rel(10_in,200);
  auto_set_shot=true;
  while (auto_set_shot) pros::delay(10);
  auto_set_angler_target(front_SP.mid+86);
  // pros::delay(750);
  pros::delay(250);
  while (!pun_ball) pros::delay(5);
  auto_set_shot = true;
