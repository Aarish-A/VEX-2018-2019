...
f.write(" \n")
f.write("#define WHILE(machineIn, condition) machineIn##Blocked = true; \\ \n")
f.write("	while( (NOT_T_O(machineIn) && machineIn##VelSafetyCount < 10) && (condition) ) \n")
f.write(" \n")
f.write("#define DO_WHILE(machineIn, condition) while( NOT_T_O(machineIn) && machineIn##VelSafetyCount < 10 && (condition) ); \\ \n")
f.write("	machineIn##Blocked = true \n")
f.write(" \n")
'''
...
f.write(" \n")
f.write("#define LOG_2(machine1, machine2) if(machine1##Logs || machine2##Logs) writeDebugStreamLine \n")
f.write(" \n")
f.write("#define VEL_CHECK_INC(machineIn, safetyType) machineIn##VelSafetyCheck(safetyType); \\ \nmachineIn##StateCycCount++ \n")
f.write("#define MACHINE_AWAIT(machineIn) while (machineIn##Blocked) sleep(10) \n")
'''
...
for cnt in range (minPNum, maxPNum+1):
	...
	f.write("bool machine##Blocked =  false; \\ \n")
	...
	f.write("void machine##StateChange(int stateIn, bool await = 0, long timeout = -1, float velSafetyThresh = -1, tVelDir velDir = -1, type1 arg1In = -1, type2 arg2In = -1) \\" + "\n" )
	f.write("{ \\" + "\n" )
	...
	f.write("		writeDebugStreamLine (\"%d %d\" #machine \"State:%d, TO:%d velS:%f, %d, %d\", npgmTime, curTime, machine##State, machine##timeout, machine##VelSafetyThresh, machine##arg1Name, machine##arg2Name); \\" + "\n" )
	...
	f.write("			} \\ \n")
	f.write("		} \\ \n")
#	f.write("		writeDebugStreamLine(\"%d Sen Vel: %f, cnt: %d\", npgmtime, out, machine##velSafetyCount); \\ \n")
	f.write("		unsigned long curTime = npgmTime; \\ \n")
	...
	f.write("					{ \\" + "\n" )
	f.write("						machine##VelSafetyCount ++; \\" + "\n" )
	f.write("						if(machine##Logs) writeDebugStreamLine(\"%d:\"#machine\"velSafety trip(dir)%f\", npgmtime, out); \\ \n")
	f.write("					} \\" + "\n" )
	f.write("				} \\" + "\n" )
	f.write("		} \\ \n")
	f.write("		tRelease(); \\ \n")
	...
	f.write("bool machine##SafetyCheck(int timedOutState = machine##state0, type1 machine##arg1Name = -1, type2 machine##arg2Name = -1) \\" + "\n" )
	f.write("{ \\" + "\n" )
	f.write("		tHog(); \\" + "\n" )
	...
	f.write("			machine##StateChange(timedOutState, false, machine##arg1Name, machine##arg2Name); \\" + "\n" )
	f.write("			return true; \\" + "\n")
	f.write("		} \\" + "\n" )
	f.write("		else \\" + "\n" )
	f.write("		{ \\" + "\n" )
